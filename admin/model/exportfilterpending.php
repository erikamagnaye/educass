...
$stmt = $conn->prepare("SELECT CONCAT(lastname, ', ', firstname, ' ' , midname ) AS fullname, 
 gender, 
    ...
    FROM student join studentcourse on student.studid=studentcourse.studid 
            ...
            WHERE application.educid=? and brgy=? and `year` LIKE ? and application.appstatus = 'Pending' 
            ORDER BY `year` ASC, lastname ASC");
...
header('Content-Disposition: attachment; filename=Pending Educational Assistance Applicants.csv');
