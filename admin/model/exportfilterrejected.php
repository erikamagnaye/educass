...
$stmt = $conn->prepare("SELECT CONCAT(lastname, ', ', firstname, ' ' , midname ) AS fullname, 
 gender, 
    ...
    FROM student join studentcourse on student.studid=studentcourse.studid 
            join application on studentcourse.courseid=application.courseid  WHERE application.educid=? and brgy=? and `year` LIKE ? and application.appstatus = 'Rejected' ORDER BY `year` ASC, lastname ASC");
...
header('Content-Disposition: attachment; filename=Rejected Educational Assistance Applicants.csv');
